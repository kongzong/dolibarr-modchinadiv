...
> 历史规格：保留 V0.1 的开发范围与验收目标。当前版本为 V0.4.0，功能见
> [README](../README.md)，联系人编码及地址规范化见 [V0.4 规格](spec-chinadiv-v0.4.md)。
> 2026-09-13 勘误：统一示例为六位编码，修正区县名称、港澳台代码与广东父级查询参数。
...
- 编码：统一使用 6 位行政区划代码（如 110108 海淀区、110105 朝阳区）
- 层级：1=省 2=市 3=区县；直辖市：市辖区层（如 110100）保留为 level 2 但标记"市辖区"名
- 港澳台：纳入（香港 810000、澳门 820000、台湾 710000），当前种子数据不含下级
...
3. 重复导入幂等（更新而非重复插入）
4. `GET /api/index.php/chinadiv/divisions?parent=440000` 返回广东下属市；父编码必须为六位，`parent=44` 会返回空数组
5. 测试全过
